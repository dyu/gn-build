...
def FillXcodeVersion(settings):
  ...
  lines = subprocess.check_output(['xcodebuild', '-version']).decode().splitlines()
...
def FillSDKPathAndVersion(settings, platform, xcode_version):
  ...
  settings['sdk_version'] = subprocess.check_output([
      'xcrun', '-sdk', platform, '--show-sdk-version']).decode().strip()
  settings['sdk_platform_path'] = subprocess.check_output([
      'xcrun', '-sdk', platform, '--show-sdk-platform-path']).decode().strip()
  ...
  if xcode_version >= '0720':
    settings['sdk_build'] = subprocess.check_output([
        'xcrun', '-sdk', platform, '--show-sdk-build-version']).decode().strip()
  else:
